...
class PlaylistUtils(private val client: OkHttpClient) {
    fun extractFromHls(playlistUrl: String, referer: String? = null): List<Video> {
        val masterPlaylist = try {
            ...
            return listOf(Video(source = playlistUrl, subtitles = emptyList(), type = "application/x-mpegURL"))
        }

        val videoList = mutableListOf<Video>()
        if (!masterPlaylist.contains("#EXT-X-STREAM-INF")) {
            videoList.add(Video(source = playlistUrl, subtitles = emptyList(), type = "application/x-mpegURL"))
            return videoList
        }
        ...
        val subtitles = SUBTITLE_REGEX.findAll(masterPlaylist).map {
            ...
            Video.Subtitle(label = it.groupValues[1], file = url)
        }.toList()
        ...
        masterPlaylist.substringAfter("#EXT-X-STREAM-INF:").split("#EXT-X-STREAM-INF:").forEach {
            ...
            videoList.add(Video(source = url, subtitles = subtitles, type = "application/x-mpegURL", headers = videoHeaders))
        }
    ...
    private fun getQualityFromBandwidth(text: String): String {
        ...
        return when { bandwidth >= 2000000 -> "1080p"; bandwidth >= 1000000 -> "720p"; bandwidth >= 600000 -> "480p"; else -> "360p" }
    }
