...
object DecryptHelper {
    ...
    fun findEncodedRegex(source: String): String? = encodedRegex.find(source)?.groupValues?.getOrNull(1)

    fun decrypt(encodedString: String): JsonObject = decryptF7(encodedString)
    ...
    private fun decryptF7(p8: String): JsonObject {
        ...
        } catch (e: Exception) { println("Decryption error: ${e.message}"); JsonObject() }
    }
    ...
    private fun replacePatterns(input: String): String {
        val patterns = listOf("@\$", "^^", "~@", "%?", "*~", "!!", "#&")
        return patterns.fold(input) { result, pattern -> result.replace(Regex(Regex.escape(pattern)), "_") }
    }
