    ...
    suspend fun servers(videoType: Video.Type): List<Video.Server> {
        ...
        val url = when (videoType) {
            is Video.Type.Episode -> {
                val id = Base64.getEncoder().encode("tmdb|series|${videoType.tvShow.id}".toByteArray()).toString(Charsets.UTF_8)
                val mediaId = try {
                    service.getResponse(
            ...
            is Video.Type.Movie -> {
                val id = Base64.getEncoder().encode("tmdb|movie|${videoType.id}".toByteArray()).toString(Charsets.UTF_8)
                "$mainUrl/api/v1/titles/$id?loader=titlePage"
            }
        }
