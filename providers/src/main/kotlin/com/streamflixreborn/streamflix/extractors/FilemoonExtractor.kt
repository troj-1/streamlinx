...
open class FilemoonExtractor : Extractor() {
    ...
    private fun decryptPlayback(data: PlaybackData): String {
        val iv = Base64.getDecoder().decode(data.iv)
        val payload = Base64.getDecoder().decode(data.payload)
        val p1 = Base64.getDecoder().decode(data.key_parts[0])
        val p2 = Base64.getDecoder().decode(data.key_parts[1])
