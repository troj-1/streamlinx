...
class VixSrcExtractor : Extractor() {
    ...
    fun server(videoType: Video.Type, lang: String = "en"): Video.Server {
        return Video.Server(
            id = name,
            name = name,
            src = when (videoType) {
                is Video.Type.Episode -> "$mainUrl/api/tv/${videoType.tvShow.id}/${videoType.season.number}/${videoType.number}?lang=$lang"
                is Video.Type.Movie -> "$mainUrl/api/movie/${videoType.id}?lang=$lang"
            },
        )
    }
    ...
    override suspend fun extract(link: String): Video {
        ...
        val providerLang = UserPreferences.providerLanguage ?: "en"
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                if (response.isSuccessful && response.body != null) {
                    ...
                    val langCode = link.substringAfter("lang=", "").substringBefore("&").ifBlank { UserPreferences.providerLanguage ?: "en" }
                    ...
                    for (line in lines) {
                        ...
                        if (patchedLine.startsWith("#EXT-X-MEDIA:TYPE=AUDIO")) {
                            ...
                            val isTargetAudio = patchedLine.contains("LANGUAGE=\"$langCode\"", ignoreCase = true) || 
                                                ...
                                                (langCode == "ru" && (patchedLine.contains("Russian", true) || patchedLine.contains("rus", true) || patchedLine.contains("Русский", true))) ||
                                                (langCode == "de" && (patchedLine.contains("German", true) || patchedLine.contains("Deutsch", true) || patchedLine.contains("ger", true) || patchedLine.contains("deu", true))) ||
                                                (langCode == "fr" && (patchedLine.contains("French", true) || patchedLine.contains("Français", true) || patchedLine.contains("fra", true) || patchedLine.contains("fre", true))) ||
                                                (langCode == "pl" && (patchedLine.contains("Polish", true) || patchedLine.contains("Polski", true) || patchedLine.contains("pol", true))) ||
                    ...
                    val base64Manifest = Base64.getEncoder().encodeToString(finalLines.joinToString("\n").toByteArray())
