...
object TvLibrefutbolProvider : IptvProvider {
    ...
    override suspend fun search(query: String, page: Int): List<Item> = try {
                    ...
                    doc.select("script").forEach { s ->
                        ...
                        if (data.contains("atob(")) {
                            try {
                                ...
                                var dec = String(Base64.getDecoder().decode(enc))
                                repeat(2) {
                                    if (dec.contains("atob(")) {
                                        val innerEnc = dec.substringAfter("atob(\"").substringBefore("\")")
                                        dec = String(Base64.getDecoder().decode(innerEnc))
                                    } else if (!dec.startsWith("http")) {
                                        try { dec = String(Base64.getDecoder().decode(dec)) } catch (_: Exception) {}
                                    }
                                }
