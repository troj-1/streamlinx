...
object PelisplustoProvider : Provider {
    ...
    private fun parseShows(document: Document): List<Item> {
        ...
        try {
            ...
            for (li in serverElements) {
                try {
                    ...
                    val decodedUrl = String(Base64.getDecoder().decode(dataServer))

                    val finalUrl = if (!decodedUrl.contains("https://")) {
                        val reEncoded = String(Base64.getEncoder().encode(dataServer.toByteArray())).trim()
